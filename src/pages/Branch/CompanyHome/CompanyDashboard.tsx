...
export const CompanyDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      {/* Welcome banner */}
      <div className="overflow-hidden rounded-xl" style={{ background: 'linear-gradient(135deg, #0F172A 0%, #1E3A8A 100%)' }}>
        <div className="flex items-center justify-between px-6 py-5">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-blue-300">Active Workspace</p>
            <h2 className="mt-1 text-xl font-black text-white">
              {activeCompany?.name || companySlug?.toUpperCase()}
            </h2>
            <p className="mt-1 text-sm text-white/50">
              Branch: <span className="font-semibold text-white/70 uppercase">{branchSlug}</span>
            </p>
          </div>
          <button
            onClick={() => navigate(`/branch/${branchSlug}/companies`)}
            className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/10 px-3.5 py-2 text-xs font-semibold text-white/80 transition-colors hover:bg-white/20 hover:text-white"
          >
            Switch Company
          </button>
        </div>

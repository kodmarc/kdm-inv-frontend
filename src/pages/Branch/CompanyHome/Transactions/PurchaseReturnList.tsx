...
export const PurchaseReturnList: React.FC = () => {
  ...
  const [returns, setReturns] = useState<PurchaseReturnItem[]>([]);
  ...
  const filteredReturns = companyReturns.filter(ret => {
    const matchesSearch = 
      ret.purchase_return_code.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <div className="animate-fade-in bg-white border border-zinc-200 rounded-2xl shadow-xs p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 pb-4 border-b border-zinc-100">
        <div>
          <h3 className="text-xl font-bold text-navy tracking-tight">Purchase Returns</h3>
        ...
        <div>
          ...
          >
            <span className="text-sm font-bold">+</span> New Purchase Return
          </button>
        </div>
      </div>
      ...
      {isListLoading ? (
        ...
        <div className="py-12 border border-dashed border-zinc-200 rounded-2xl text-center text-xs text-muted font-medium bg-zinc-50/20">
          No purchase return vouchers found matching the filters.
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-zinc-100 text-xs font-medium text-zinc-700">
              {filteredReturns.map((ret) => (
                <tr key={ret.id} className="hover:bg-zinc-50/40 transition-colors">
                  <td className="px-4 py-3.5 text-navy font-bold">{ret.purchase_return_code}</td>

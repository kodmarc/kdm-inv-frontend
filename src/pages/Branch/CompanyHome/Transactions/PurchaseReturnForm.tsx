...
const PurchaseReturnForm: React.FC = () => {
  ...
  // Snapshot/Display Fields from Supplier
  ...
  const handleRowItemChange = (index: number, itemId: string) => {
    ...
    updated[index] = {
      ...
      rate: rate,
      amount: '0.00',
      discount_amount: updated[index].discount_amount || '0.00',
    ...
    const parsedDiscount = parseFloat(updated[index].discount_amount) || 0;
    ...
    const taxableAmount = Math.max(0, baseAmount - parsedDiscount - toAmt);
    const sTaxAmt = taxableAmount * (parsedSTaxRate / 100);
    const netAmt = taxableAmount + sTaxAmt;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      adv_income_tax: advIncomeTax || '0.00',
      ...
      <div className="mb-6 pb-4 border-b border-zinc-100 flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-navy tracking-tight">
            {id ? 'Modify Purchase Return' : 'New Purchase Return'}
          </h3>
          <p className="text-xs text-muted mt-1 font-medium">
            Record returned products, adjust tax balances, and post ledger adjustments.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={() => navigate(`/branch/${branchSlug}/company/${companySlug}/purchase-return`)}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 pt-4">
          <div className="md:col-span-7 border border-zinc-200 rounded-2xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-zinc-500 uppercase">Freight charges (Rs.)</label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-zinc-500 uppercase">Adv. Income Tax (Rs.)</label>
                <input
                  type="number"
                  step="0.01"
                  className="text-xs border border-zinc-200 rounded-xl px-3 py-2 bg-white font-bold outline-hidden"
          ...
          <div className="md:col-span-5 border border-zinc-200 rounded-2xl p-5 space-y-3.5 text-xs text-zinc-700 font-semibold">
            ...
            <div className="flex justify-between items-center text-zinc-600">
              <span>Sales Tax Reversals</span>
              <span>Rs. {parseFloat(sTax).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>

            <div className="flex justify-between items-center text-zinc-600">
              <span>Freight Adjustment</span>
              <span>Rs. {parsedFreight.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>

            <div className="flex justify-between items-center text-zinc-600">
              <span>Advance Income Tax Adj</span>
              <span>Rs. {parsedAdvTax.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>
        ...
        <div className="pt-4 border-t border-zinc-100 flex items-center justify-end gap-3">
          ...
          <button
            ...
            disabled={isSubmitting}
          >
            {isSubmitting ? 'Saving Return...' : 'Save Return Sheet'}
          </button>
        </div>
      </form>
    </div>

...
interface DamageReturnLineItem {
  id?: string;
  item: string;
  item_name?: string;
  item_code?: string;
...
interface DamageReturnItem {
  id: string;
  damage_return_code: string;
  date: string;
  party_inv_no?: string;
  supplier: string;
  ...
  line_items: DamageReturnLineItem[];
  created_at: string;
  updated_at: string;
}
...
export const DamageReturnList: React.FC = () => {
  ...
  const [returns, setReturns] = useState<DamageReturnItem[]>([]);
  ...
  return (
    <div className="animate-fade-in bg-white border border-zinc-200 rounded-2xl shadow-xs p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 pb-4 border-b border-zinc-100">
        <div>
          <h3 className="text-xl font-bold text-navy tracking-tight">Damage Returns</h3>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <span className="text-sm font-bold">+</span> New Damage Return
          </button>
        </div>
      </div>
      ...
      {isListLoading ? (
        ...
        <div className="py-12 border border-dashed border-zinc-200 rounded-2xl text-center text-xs text-muted font-medium bg-zinc-50/20">
          No damage return vouchers found matching the filters.
        </div>
      ) : (
          ...
          <table className="min-w-full divide-y divide-zinc-200 text-left">
            <thead>
              <tr className="bg-zinc-50/70">
                ...
                <th className="px-4 py-3 text-[10px] font-bold tracking-wider text-muted uppercase">Party Inv. #</th>
            ...
            <tbody className="divide-y divide-zinc-100 text-xs font-medium text-zinc-700">
              {filteredReturns.map((ret) => (
                <tr key={ret.id} className="hover:bg-zinc-50/40 transition-colors">
                  <td className="px-4 py-3.5 text-navy font-bold">{ret.damage_return_code}</td>
                  <td className="px-4 py-3.5 text-zinc-500">{ret.date}</td>
                  <td className="px-4 py-3.5 text-zinc-500">{ret.party_inv_no || '—'}</td>
                  <td className="px-4 py-3.5 font-semibold text-zinc-800">{ret.supplier_name}</td>

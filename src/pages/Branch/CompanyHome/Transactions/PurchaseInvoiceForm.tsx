...
export const PurchaseInvoiceForm: React.FC = () => {
  ...
  useEffect(() => {
    if (id && purchaseInvoices.length > 0) {
  ...
  }, [id, purchaseInvoices, items]);

  // Handle supplier select change to load snapshots
  ...
  // Sync document-level s_tax state with line item sales tax sum
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (id) {
        ...
        setSuccess('Purchase invoice generated successfully.');
      }
      fetchPurchaseInvoices();
      setTimeout(() => setSuccess(''), 3000);
      navigate(`/branch/${branchSlug}/company/${companySlug}/purchase-invoice`);
    } catch (err: any) {
      ...
      <div className="mb-6 pb-4 border-b border-zinc-100 flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-navy tracking-tight">
            {id ? 'Modify Purchase Invoice' : 'New Purchase Invoice'}
          </h3>
          <p className="text-xs text-muted mt-1 font-medium">
            Record supplier invoices, update purchasing accounts, and check stock additions.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={() => navigate(`/branch/${branchSlug}/company/${companySlug}/purchase-invoice`)}
        ...
        <div className="flex justify-end items-start pt-4">
          <div className="border border-zinc-200 rounded-2xl p-5 w-full max-w-md space-y-3.5 text-xs text-zinc-700 font-semibold">
            ...
            <div className="flex justify-between items-center py-1 border-t border-b border-dashed border-zinc-200">
              <span>Freight Charges (Rs.)</span>
              <input
                type="number"
                step="0.01"
                min="0"
                className="w-28 text-right text-xs border border-zinc-200 rounded-lg px-2 py-1 bg-white font-bold text-zinc-800 outline-hidden"
                value={freight}
            ...
            {/* Advance Income Tax */}
            <div className="flex justify-between items-center pb-2 border-b border-zinc-100">
              <span>Advance Income Tax (Rs.)</span>
              <input
                type="number"
                step="0.01"
                min="0"
